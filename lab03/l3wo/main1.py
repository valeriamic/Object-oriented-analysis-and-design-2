...
# --- ФУНКЦИЯ ДЛЯ ПУТЕЙ (КРИТИЧНО ДЛЯ EXE) ---
def resource_path(relative_path):
    """ Получает абсолютный путь к ресурсу, работает и при запуске .py, и внутри .exe """
    try:
...
# Базовый путь для поиска файлов в папке скрипта
...
class Villager:
    ...
    def hit(self):
        if self.hpBar > 0:
            self._is_hit = True
            self.hpBar -= 2
            # Увеличиваем цены во всех слотах при ударе
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Villager Shop - Stars & GUI")
        self.clock = pygame.time.Clock()
        self.font = pygame.font.SysFont("Arial", 22, bold=True)
        self.game_over = False

        # Эффекты покупки
        self.purchase_timer = 0
        self.purchase_text = ""
    ...
    def _load_resources(self):
        def load_img(name, w, h):
            # Сначала пробуем resource_path (для EXE), затем BASE_PATH (для .py)
            path = resource_path(name)
            try:
                img = pygame.image.load(path).convert_alpha()
                return pygame.transform.scale(img, (w, h))
            except:
                print(f"Файл {name} не найден, создана заглушка")
        ...
        self.img_stars = load_img("stars.png", 100, 100) # Загрузка звезд
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    if self.villager.takeEmeralds(active_trade.get_current_price()):
                        # Активация эффекта покупки
                        self.purchase_text = f"Куплено: {active_trade.itemName}"
                        self.purchase_timer = 90 # 1.5 секунды
    ...
    def draw(self):
        if not self.game_over:
            ...
            # Центральный спрайт
            img = self.item_images[active_trade.itemName] if self.show_item else self.img_villager
            ...
            # --- ЭФФЕКТ ЗВЕЗД ---
            if self.purchase_timer > 0:
                stars_rect = self.img_stars.get_rect(center=(self.center_pos[0], img_rect.top - 80))
                self.screen.blit(self.img_stars, stars_rect)
                
                buy_surf = self.font.render(self.purchase_text, True, (0, 255, 0))
                self.screen.blit(buy_surf, (self.WIDTH//2 - buy_surf.get_width()//2, 120))
                self.purchase_timer -= 1
