...
namespace RE9 {
	...
	public interface class IInputMaterial { int GetVolume(); };
	...
	public ref class BloodToAmmoAdapter : public IInputMaterial {
	Blood^ b; public: BloodToAmmoAdapter(Blood^ _b) { b = _b; }
					virtual int GetVolume() { return b->ExtractIron(); }
	};

	public ref class BloodToHealAdapter : public IInputMaterial {
	Blood^ b; public: BloodToHealAdapter(Blood^ _b) { b = _b; }
					virtual int GetVolume() { return b->GetAlcohol(); }
	};

	public ref class BloodToExplosiveAdapter : public IInputMaterial {
	Blood^ b; public: BloodToExplosiveAdapter(Blood^ _b) { b = _b; }
					virtual int GetVolume() { return b->GetAntibodies(); }
	};

public ref class AmmoPress { public: bool Craft(IInputMaterial^ m, int c) { return m->GetVolume() >= (c * 5); } };
public ref class MedLab { public: bool Produce(IInputMaterial^ m, int c) { return m->GetVolume() >= (c * 30); } };
public ref class ChemStation { public: bool Create(IInputMaterial^ m, int c) { return m->GetVolume() >= (c * 60); } };
									 ...
									 private:
										 ...
										 void OnAmmoClick(Object^ sender, EventArgs^ e) {
											 ...
											 if (c > 0 && ammoPress->Craft(gcnew BloodToAmmoAdapter(bloodSource), c)) {
										 ...
										 void OnHealClick(Object^ sender, EventArgs^ e) {
											 ...
											 if (c > 0 && medLab->Produce(gcnew BloodToHealAdapter(bloodSource), c)) {
										 ...
										 void OnExploClick(Object^ sender, EventArgs^ e) {
											 ...
											 if (c > 0 && chemStation->Create(gcnew BloodToExplosiveAdapter(bloodSource), c)) {
