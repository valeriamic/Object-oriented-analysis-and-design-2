...
namespace RE9 {
	...
	interface class IInputMaterial;
	ref class Blood;
	ref class AmmoPress;
	ref class MedLab;
	...
	inline void MyForm::OnAmmoClick(Object^ sender, EventArgs^ e) {
		...
		if (count <= 0) return;
		// Здесь создается адаптер, теперь тип определен
		if (ammoPress->Craft(gcnew BloodToAmmoAdapter(bloodSource), count)) {
	...
	inline void MyForm::OnHealClick(Object^ sender, EventArgs^ e) {
		...
		if (count <= 0) return;
		if (medLab->Produce(gcnew BloodToHealAdapter(bloodSource), count)) {
	...
	inline void MyForm::OnExploClick(Object^ sender, EventArgs^ e) {
		...
		if (count <= 0) return;
		if (chemStation->Create(gcnew BloodToExplosiveAdapter(bloodSource), count)) {
